...
brandRouter.post('/', verifyToken, specificRolesOnly([RoleName.ADMIN, RoleName.SUPERADMIN]), async (request: Request, response: Response) => {
    try {
        ...
        response.status(StatusCodes.CREATED).send(new SuccessResponse(await new BrandManagement().brandById(persistedBrand.id), 'Brand created successfully', StatusCodes.CREATED))
    } catch (error: any) {
